...
class ThuatToanRSA:
    ...
    def tim_khoa_cong_khai_e(phi, so_luong=1):
        goi_y = []
        vong_lap = 0
        while len(goi_y) < so_luong and vong_lap < 500:
            vong_lap += 1
            test_e = random.randint(3, min(phi - 1, 999999))
            if test_e % 2 != 0 and ThuatToanRSA.uoc_chung_lon_nhat(test_e, phi) == 1:
                goi_y.append(test_e)
        if not goi_y:
            goi_y.append(65537 if phi > 65537 else 3)
        return goi_y
...
class UngDungChuKySoRSA:
    ...
    def chuyen_tab(self, id_man_hinh):
        ...
        tieu_de_trang = {
            ...
            2: "TẠO CHỮ KÝ SỐ TRÊN TÀI LIỆU",
            3: "GIÁM ĐỊNH & THẨM ĐỊNH CHỮ KÝ SỐ"
        }
    ...
    def tao_tab_quan_ly_khoa(self):
        ...
        c_input.grid(row=0, column=0, sticky="ew", pady=(0, 20))
        c_input.columnconfigure((1, 3, 5), weight=1)
        ctk.CTkLabel(c_input, text="⚙️ THAM SỐ ĐẦU VÀO GIẢI THUẬT", font=ctk.CTkFont(size=14, weight="bold"), text_color=self.accent_blue).grid(row=0, column=0, columnspan=6, sticky="w", padx=25, pady=(20, 15))
        ctk.CTkLabel(c_input, text="Số nguyên tố P:", font=ctk.CTkFont(size=12), text_color=self.text_title).grid(row=1, column=0, sticky="w", padx=(25, 10), pady=10)
        self.txt_p = ctk.CTkEntry(c_input, height=35, placeholder_text="Nhập P...", border_color=self.border_card, fg_color=self.bg_main, text_color=self.text_title)
        self.txt_p.grid(row=1, column=1, padx=10, pady=10, sticky="ew")
        ctk.CTkLabel(c_input, text="Số nguyên tố Q:", font=ctk.CTkFont(size=12), text_color=self.text_title).grid(row=1, column=2, sticky="w", padx=(20, 10), pady=10)
        self.txt_q = ctk.CTkEntry(c_input, height=35, placeholder_text="Nhập Q...", border_color=self.border_card, fg_color=self.bg_main, text_color=self.text_title)
        self.txt_q.grid(row=1, column=3, padx=10, pady=10, sticky="ew")
        ctk.CTkLabel(c_input, text="Khóa Công Khai E:", font=ctk.CTkFont(size=12), text_color=self.text_title).grid(row=1, column=4, sticky="w", padx=(20, 10), pady=10)
        self.txt_e = ctk.CTkEntry(c_input, height=35, placeholder_text="Nhập E...", border_color=self.border_card, fg_color=self.bg_main, text_color=self.text_title)
        self.txt_e.grid(row=1, column=5, padx=(10, 25), pady=10, sticky="ew")
        self.txt_p.bind("<Return>", lambda event: self.txt_q.focus_set())
        self.txt_q.bind("<Return>", lambda event: self.txt_e.focus_set())
        self.txt_e.bind("<Return>", lambda event: self.tao_cap_khoa_rsa())
        f_buttons = ctk.CTkFrame(c_input, fg_color="transparent")
        f_buttons.grid(row=2, column=0, columnspan=6, sticky="ew", padx=25, pady=(15, 20))
        ctk.CTkButton(f_buttons, text="🎲 Sinh Ngẫu Nhiên P, Q", width=180, height=38, font=ctk.CTkFont(weight="bold"), fg_color=("#9CA3AF", "#4B5563"), text_color="white", hover_color=("#4B5563", "#374151"), command=self.sinh_ngau_nhien_p_q).pack(side="left", padx=(0, 12))
        ctk.CTkButton(f_buttons, text="⚡ Tìm Khóa E Khả Dụng", width=180, height=38, fg_color="transparent", border_width=1, border_color=self.border_card, text_color=self.text_title, hover_color=("#E5E7EB", "#2D3748"), command=self.tim_e_hop_le).pack(side="left")
        ctk.CTkButton(f_buttons, text="🔒 TẠO HỆ THỐNG CẶP KHÓA", width=220, height=38, font=ctk.CTkFont(weight="bold"), fg_color=self.accent_green, text_color="white", hover_color="#059669", command=self.tao_cap_khoa_rsa).pack(side="right")
        ...
        ctk.CTkLabel(c_output, text="📋 THÔNG TIN CẶP KHÓA ĐANG LƯU TRONG BỘ NHỚ", font=ctk.CTkFont(size=14, weight="bold"), text_color=self.accent_green).grid(row=0, column=0, sticky="w", padx=25, pady=(20, 15))
        ctk.CTkLabel(c_output, text="Modulo (N):", font=ctk.CTkFont(weight="bold"), text_color=self.text_title).grid(row=1, column=0, sticky="w", padx=25, pady=(5, 5))
        self.txt_display_n = ctk.CTkTextbox(c_output, height=75, font=("Consolas", 12), fg_color=self.bg_main, border_color=self.border_card, border_width=1, corner_radius=8, text_color=self.text_title)
        self.txt_display_n.grid(row=2, column=0, sticky="ew", padx=25, pady=2)
        ctk.CTkLabel(c_output, text="Cặp khóa công khai (N,E):", font=ctk.CTkFont(weight="bold"), text_color=("#DC2626", "#FCA5A5")).grid(row=3, column=0, sticky="w", padx=25, pady=(5, 5))
        self.txt_display_e = ctk.CTkTextbox(c_output, height=75, font=("Consolas", 12), fg_color=self.bg_main, border_color=self.border_card, border_width=1, corner_radius=8, text_color=self.text_title)
        self.txt_display_e.grid(row=4, column=0, sticky="ew", padx=25, pady=2)
        ctk.CTkLabel(c_output, text="Cặp khóa Bí Mật (N,D):", font=ctk.CTkFont(weight="bold"), text_color=("#DC2626", "#FCA5A5")).grid(row=5, column=0, sticky="w", padx=25, pady=(5, 5))
        self.txt_display_d = ctk.CTkTextbox(c_output, height=75, font=("Consolas", 12), fg_color=self.bg_main, border_color=self.border_card, border_width=1, corner_radius=8, text_color=self.text_title)
        self.txt_display_d.grid(row=6, column=0, sticky="ew", padx=25, pady=2)
        f_file_actions = ctk.CTkFrame(c_output, fg_color="transparent")
        f_file_actions.grid(row=7, column=0, sticky="ew", pady=(25, 20), padx=25)
        ctk.CTkButton(f_file_actions, text="📁 Nhập Khóa Từ File Cũ", height=35, fg_color=("#6B7280", "#374151"), text_color="white", hover_color=("#4B5563", "#1F2937"), command=self.mo_cap_khoa).pack(side="left", padx=(0, 12))
        ctk.CTkButton(f_file_actions, text="💾 Xuất Cặp Khóa Ra File ", height=35, fg_color=("#6B7280", "#374151"), text_color="white", hover_color=("#4B5563", "#1F2937"), command=self.luu_cap_khoa).pack(side="left")
        return f_main
    ...
    def tao_tab_ky_so(self):
        ...
        ctk.CTkLabel(c_doc, text="📄 NỘI DUNG VĂN BẢN NGUỒN CẦN KÝ SỐ ", font=ctk.CTkFont(size=14, weight="bold"), text_color=self.accent_blue).pack(anchor="w", padx=25, pady=(20, 8))
        ...
        ctk.CTkLabel(c_sig, text="🔑 CHỮ KÝ SỐ ĐẠI DIỆN ĐẦU RA (MÃ SỐ S)", font=ctk.CTkFont(size=14, weight="bold"), text_color="#F87171").pack(anchor="w", padx=25, pady=(20, 8))
        ...
        ctk.CTkLabel(f_header, text="Mã Chữ Ký S", font=ctk.CTkFont(size=11, weight="bold"), width=250, anchor="w").grid(row=0, column=3, padx=5, sticky="w")
    ...
    def tao_tab_xac_minh(self):
        f_main = ctk.CTkFrame(self.container, fg_color="transparent")
        f_main.columnconfigure(0, weight=1)
        f_main.rowconfigure(3, weight=1)
        ...
        c_pub.columnconfigure(1, weight=3)
        c_pub.columnconfigure(3, weight=1)
        ctk.CTkLabel(c_pub, text="🔑 CẤU HÌNH KHÓA ĐỐI CHỨNG CÔNG KHAI", font=ctk.CTkFont(size=14, weight="bold"), text_color=self.accent_blue).grid(row=0, column=0, columnspan=4, sticky="w", padx=25, pady=(20, 12))
        ctk.CTkLabel(c_pub, text="Modulo N:", font=ctk.CTkFont(size=12), text_color=self.text_title).grid(row=1, column=0, sticky="w", padx=(25, 10), pady=15)
        self.txt_t3_n = ctk.CTkEntry(c_pub, height=35, placeholder_text="Nhập Modulo N để kiểm định...", border_color=self.border_card, fg_color=self.bg_main, text_color=self.text_title)
        self.txt_t3_n.grid(row=1, column=1, padx=10, pady=15, sticky="ew")
        ctk.CTkLabel(c_pub, text="Chỉ số E:", font=ctk.CTkFont(size=12), text_color=self.text_title).grid(row=1, column=2, sticky="w", padx=(20, 10), pady=15)
        ...
        self.txt_t3_e.grid(row=1, column=3, padx=(10, 25), pady=15, sticky="ew")
    ...
    def sinh_ngau_nhien_p_q(self):
        p = ThuatToanRSA.sinh_so_nguyen_to(10**10, 10**12)
        while True:
            q = ThuatToanRSA.sinh_so_nguyen_to(10**10, 10**12)
            if p != q: break
    ...
    def tai_van_ban_can_ky(self):
        path = filedialog.askopenfilename(filetypes=[("All Files", "*.txt *.docx *.xlsx"), ("Text Files", "*.txt")])
        if path:
    ...
    def luu_van_ban_goc(self):
        ...
        if not t: return
        path = filedialog.asksaveasfilename(defaultextension=".txt", filetypes=[("Text Document", "*.txt")])
        if path:
    ...
    def ky_so_van_ban(self):
        if self.n == 0 or self.d == 0:
            messagebox.showwarning(
                "Lỗi khóa",
                "Vui lòng tạo hệ thống cặp khóa tại Tab 1 trước!"
            )
            return
        ...
        if not chuoi_ky:
            messagebox.showwarning(
                "Thiếu dữ liệu",
                "Vui lòng nhập hoặc tải văn bản trước khi ký số!"
            )
            self.txt_source_text.focus_set()
            return
        hash_val = hashlib.sha256(chuoi_ky.encode("utf-8")).hexdigest()
        m = int(hash_val, 16) % self.n
        s_ket_qua = pow(m, self.d, self.n)
        self.original_text = chuoi_ky
        self.original_sig = s_ket_qua
        self.txt_output_sig.delete("1.0", tk.END); self.txt_output_sig.insert(tk.END, str(s_ket_qua))
        ...
        btn_select = ctk.CTkButton(
            f_row, text="", fg_color="transparent", hover_color=("#D1D5DB", "#374151"), corner_radius=5,
            command=lambda idx=self.dem_ls: self.chon_lich_su_ky(idx)
        )
    ...
    def tai_van_ban_xac_minh(self):
        ...
        if path:
            ...
            self.lbl_status_file_tab3.configure(text=f"Đã nạp: {path.split('/')[-1]}", text_color=self.accent_blue)
    ...
    def tai_chu_ky_xac_minh(self):
        ...
        if path:
            ...
            self.lbl_status_sig_tab3.configure(text=f"Đã nạp chữ ký: {path.split('/')[-1]}", text_color=self.accent_blue)
    ...
    def xac_minh_chu_ky(self):
        ...
        hash_ht = int(hashlib.sha256(van_ban_ht.encode("utf-8")).hexdigest(), 16) % n
        m2 = pow(s_ht, e, n)
